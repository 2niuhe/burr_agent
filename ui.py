...
try:
    ...
    from vibe_workflow import ApplicationState, VibeStep, application as build_application
        ...
        if workflows_file.exists():
            ...
            self.workflow_templates = [
                WorkflowTemplate(
                    ...
                    steps=[
                        {"goal": "读取和分析代码文件", "hint": "使用 read_file 工具，关注代码结构和逻辑"},
                        {"goal": "检查代码质量和潜在问题", "hint": "查找常见的编程错误、性能问题等"},
                        {"goal": "提供具体的改进建议", "hint": "给出可操作的建议，包括具体的代码修改"},
                    ]
                ),
                WorkflowTemplate(
                    name="文档生成器",
                    ...
                    steps=[
                        {"goal": "扫描项目结构", "hint": "使用 list_dir 和相关工具了解项目组织"},
                        {"goal": "分析主要代码文件", "hint": "重点分析核心模块和API接口"},
                        {"goal": "生成结构化文档", "hint": "创建 README.md 和 API 文档"},
                    ]
                )
            ]
            self.save_workflows()
    ...
    try:
        ...
        try:
            from agent.actions import (
                router, vibe_planner, vibe_step_executor,
                execute_tools, vibe_result_analyzer, step_summarizer, exit_chat
            )
            # 使用UI版本的actions
            prompt = ui_prompt
            ...
            from vibe_workflow import (
                router, exit_chat, vibe_planner, vibe_step_executor, execute_tools
            )
            # 使用UI版本的prompt
            prompt = ui_prompt
...
async def ui_chat_response(state: ApplicationState) -> Tuple[dict, Optional[ApplicationState]]:
...
def workflow_management_page():
    ...
    with ui.column().classes('w-full h-screen'):
        ...
        with ui.row().classes('w-full items-center justify-between p-4 bg-blue-100'):
            ...
            ui.button('进入对话模式', on_click=lambda: ui.navigate.to('/chat')).classes('bg-green-500 text-white')
        ...
        with ui.row().classes('w-full flex-1'):
            ...
            with ui.column().classes('w-1/3 p-4 border-r'):
                with ui.row().classes('w-full items-center justify-between mb-4'):
                    ...
                    ui.button('+ 创建新工作流', on_click=create_new_workflow).classes('bg-blue-500 text-white')
            ...
            with ui.column().classes('w-2/3 p-4'):
                ...
                with workflow_editor_container:
                    ui.label('选择一个工作流进行编辑，或创建新的工作流').classes('text-gray-500 text-center mt-20')
            ...
            with ui.card().classes('w-full p-3'):
                with ui.row().classes('w-full items-center justify-between'):
                    ...
                    with ui.row().classes('space-x-2'):
                        ui.button('运行', on_click=lambda idx=i: run_workflow(idx)).classes('bg-green-500 text-white text-xs')
                        ui.button('编辑', on_click=lambda idx=i: edit_workflow(idx)).classes('bg-blue-500 text-white text-xs')
                        ui.button('删除', on_click=lambda idx=i: delete_workflow(idx)).classes('bg-red-500 text-white text-xs')
...
def edit_workflow(index: int):
    ...
    with ui_state.workflow_editor:
        ...
        with ui.column().classes('w-full space-y-4'):
            ...
            desc_input = ui.textarea('工作流描述', value=workflow.description).classes('w-full')
                    ...
                    with steps_container:
                        with ui.card().classes('w-full p-3'):
                            with ui.row().classes('w-full items-center justify-between mb-2'):
                                ...
                                ui.button('删除', on_click=lambda idx=i: remove_step(idx)).classes('bg-red-500 text-white text-xs')
                            
                            step_goal = ui.input('目标', value=step.get('goal', '')).classes('w-full mb-2')
                            step_hint = ui.textarea('提示 (可选)', value=step.get('hint', '')).classes('w-full')
                            
                            # 绑定更新事件
                            step_goal.on('blur', lambda e, idx=i: update_step_goal(idx, e.sender.value))
                            step_hint.on('blur', lambda e, idx=i: update_step_hint(idx, e.sender.value))
            ...
            # 添加步骤按钮
            ui.button('+ 添加步骤', on_click=add_step).classes('bg-blue-500 text-white mt-2')
            ...
            ui.button('保存工作流', on_click=save_workflow).classes('bg-green-500 text-white mt-4')
            ...
            with ui.card():
                ...
                with ui.row().classes('justify-end mt-4'):
                    ...
                    def confirm_delete():
                        ...
                        dialog.close()
                        ui.navigate.reload()
                    ui.button('删除', on_click=confirm_delete).classes('bg-red-500 text-white')
...
def chat_execution_page():
    ...
    with ui.column().classes('w-full h-screen'):
        ...
        with ui.row().classes('w-full items-center justify-between p-4 bg-green-100'):
            ...
            ui.button('返回管理', on_click=lambda: ui.navigate.to('/')).classes('bg-blue-500 text-white')
        ...
        with ui.row().classes('w-full flex-1'):
            ...
            with ui.column().classes('w-1/5 p-4 border-r bg-gray-50'):
                ...
                # 当前目标显示
                ui_state.current_goal_display = ui.label('').classes('text-sm text-gray-600 mb-4 p-2 bg-white rounded')
            ...
            with ui.column().classes('w-3/5 p-4'):
                ...
                # 聊天容器
                ui_state.chat_container = ui.column().classes('w-full flex-1 overflow-y-auto border rounded p-4 bg-white')
                
                # 输入区域
                with ui.row().classes('w-full mt-4'):
                    chat_input = ui.input('输入您的请求...').classes('flex-1')
                    send_button = ui.button('发送', on_click=lambda: send_message(chat_input.value, chat_input)).classes('bg-blue-500 text-white')
                
                # 绑定回车事件
                chat_input.on('keydown.enter', lambda: send_message(chat_input.value, chat_input))
    ...
    if ui_state.current_workflow:
        ui_state.current_goal_display.text = f"目标: {ui_state.current_workflow.initial_goal}"
        ...
        for i, step in enumerate(ui_state.app_state.vibe_plan):
            ...
            with ui_state.stepper:
                with ui.step(f'步骤 {step.step_id + 1}'):
                    ...
                    if step.analysis:
                        ui.label(f'结果: {step.analysis}').classes('text-xs text-gray-600')
...
async def process_user_input(message: str):
    ...
    try:
        ...
        if ui_state.execution_mode_switch and hasattr(ui_state.execution_mode_switch, 'value'):
            execution_mode = "interactive" if ui_state.execution_mode_switch.value else "yolo"
        ...
        try:
            # 直接设置用户输入
            setattr(ui_state.burr_app.state, 'user_input', actual_input)
            setattr(ui_state.burr_app.state, 'execution_mode', execution_mode)
            setattr(ui_state.burr_app.state, 'current_goal', ui_state.app_state.current_goal)
            setattr(ui_state.burr_app.state, 'vibe_plan', ui_state.app_state.vibe_plan)
            setattr(ui_state.burr_app.state, 'chat_history', ui_state.app_state.chat_history)
...
def update_ui_from_burr_state():
    ...
    try:
        ...
        if isinstance(burr_state, ApplicationState):
            ...
            logger.warning(f"Burr state is not an ApplicationState instance, but {type(burr_state)}")
            return

        # 更新UI组件
        if ui_state.current_goal_display:
            ui_state.current_goal_display.text = f"目标: {ui_state.app_state.current_goal}"
...
def show_tool_confirmation():
    ...
    # 在聊天容器中添加工具确认卡片
    with ui_state.chat_container:
        with ui.card().classes('w-full p-4 bg-yellow-100 border-yellow-400') as confirmation_card:
            ...
            for tool_call in ui_state.app_state.pending_tool_calls:
                with ui.row().classes('w-full mb-2'):
                    ...
                    ui.label(f"{tool_call.function.name}({tool_call.function.arguments})").classes('text-sm font-mono')
            ...
            with ui.row().classes('w-full justify-end mt-4 space-x-2'):
                ...
                approve_btn = ui.button('✅ 批准', on_click=approve_handler).classes('bg-green-500 text-white')
                deny_btn = ui.button('❌ 拒绝', on_click=deny_handler).classes('bg-red-500 text-white')
...
async def approve_tools():
    ...
    try:
        # 设置批准状态
        setattr(ui_state.burr_app.state, 'tool_execution_allowed', True)
...
async def deny_tools():
    ...
    try:
        # 设置拒绝状态
        setattr(ui_state.burr_app.state, 'tool_execution_allowed', False)
